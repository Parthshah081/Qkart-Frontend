...
const Checkout = () => {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      ...
      const availBalance = parseInt(localStorage.getItem("balance") - getTotalCartValue(items));
      localStorage.setItem("balance", availBalance); 
      ...
      if(e.response){
        ...
        enqueueSnackbar(
          "Could not place order. Check that backend is running, reachable and return valid JSON.",
          {
            variant: "error"
          }
        )
  ...
  useEffect(() => {
    const fetchAddress = async () => {
      if(!token){
        enqueueSnackbar("Please log in to view this page", {
          variant: "info"
        })
              ...
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               {addresses.all.length > 0 ? (
                addresses.all.map((address) => (
                  <Box onClick={() => setAddresses({...addresses, selected: address._id})} style={{cursor: "pointer", backgroundColor: address._id === addresses.selected ? "#E9F5E1" : "inherit"}} key={address._id} className={addresses.selected === address._id ? "selected" : "", "not-selected address-item"}>
